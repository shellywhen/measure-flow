...
(function (networkcube) {
    ...
    var DynamicGraph = (function () {
        ...
        DynamicGraph.prototype.initDynamicGraph = function (data) {
          ...
          let timeFormat = 'timeFormat' in data ? data.timeFormat : TIME_FORMAT
            this.clearSelections();
            this.name = data.name;
            this.gran_min = 0;
            this.gran_max = 0;
            if (networkcube.isValidIndex(data.linkSchema.time)) {
                ...
                for (var i = 0; i < data.linkTable.length; i++) {
                    ...
                    unixTime = parseInt(moment(timeLabel, timeFormat).format('x'));
            ...
            for (var i = 0; i < data.nodeTable.length; i++) {
                ...
                if (networkcube.isValidIndex(data.nodeSchema.time)) {
                    ...
                    if (timeLabel == undefined) {
                        ...
                        time = this._times[this.getTimeIdForUnixTime(parseInt(moment(timeLabel, timeFormat).format('x')))];
                    }
                }
                else {
                    time = this._times[0];
            ...
            for (var i = 0; i < data.linkTable.length; i++) {
                ...
                if (this.linkArrays.id.indexOf(linkId) == -1) {
                    ...
                    // ------------------------------------------------- Shelly adds "index of"
                    this.linkArrays.source[linkId] = this.nodeArrays.id.indexOf(row[data.linkSchema.source]);
                    this.linkArrays.target[linkId] = this.nodeArrays.id.indexOf(row[data.linkSchema.target]);
                    // --------------------------------------------------
    ...
    var DataManager = (function () {
        ...
        DataManager.prototype.importData = function (session, data) {
            ...
            if (this.isSchemaWellDefined(data)) {
                ...
                console.log(this.isSchemaWellDefined)
...
(function (networkcube) {
    function searchForTerm(term, dgraph, type) {
        ...
        console.log('inside networkcube', result)
        return result;
    }
